...
func (r RequestSchemaValidator) GetClusterNamespaceCombinations(orgName string, teamName string, gitRepoUrl string, gitCommitHash string) ClusterNamespaceGroups {
	data := r.getPipelineData(orgName, teamName, gitRepoUrl, gitCommitHash)
	marked := make(map[string]bool)
	var groups ClusterNamespaceGroups

	for _, step := range data.Steps {
		payload, _ := r.GetStepApplicationPayload(orgName, teamName, gitRepoUrl, gitCommitHash, step.Name)
