...
func New(conn *redigomock.Conn) db.DbOperator {
	pool := &redis.Pool{
		Dial:    func() (redis.Conn, error) { return conn, nil },
		MaxIdle: 10,
	}
	return &MockRedisClientOperator{pool: pool}
}

func (r *MockRedisClientOperator) GetClient() db.DbClient {
	conn := r.pool.Get()
	return &MockRedisClientImpl{client: conn}
}

func (r *MockRedisClientOperator) Close() {
	r.pool.Close()
}
...
func (r *MockRedisClientImpl) Close() {
	r.client.Close()
}

func (r *MockRedisClientImpl) StoreValue(key string, schema interface{}) {
	r.store(key, schema, none)
}

func (r *MockRedisClientImpl) InsertValueInList(key string, schema interface{}) {
	r.store(key, schema, insert)
}

func (r *MockRedisClientImpl) UpdateHeadInList(key string, schema interface{}) {
	r.store(key, schema, update)
}

func (r *MockRedisClientImpl) InsertValueInTransactionlessList(key string, object interface{}) {
...
func (r *MockRedisClientImpl) UpdateHeadInTransactionlessList(key string, object interface{}) {
...
func (r *MockRedisClientImpl) store(key string, object interface{}, listStoreOperation ListStoreOperation) {
...
func (r *MockRedisClientImpl) FetchNotification(key string) clientrequest.Notification {
...
func (r *MockRedisClientImpl) FetchPipelineInfoList(key string, increment int) []auditlog.PipelineInfo {
...
func (r *MockRedisClientImpl) FetchLatestPipelineInfo(key string) auditlog.PipelineInfo {
...
func (r *MockRedisClientImpl) FetchTeamSchema(key string) team.TeamSchema {
...
func (r *MockRedisClientImpl) FetchClusterSchema(key string) cluster.ClusterSchema {
...
func (r *MockRedisClientImpl) FetchClusterSchemaTransactionless(key string) cluster.ClusterSchema {
...
func (r *MockRedisClientImpl) FetchStringList(key string) []string {
...
func (r *MockRedisClientImpl) FetchLogList(key string, increment int) []auditlog.Log {
...
func (r *MockRedisClientImpl) FetchLatestLog(key string) auditlog.Log {
...
func (r *MockRedisClientImpl) FetchLatestDeploymentLog(key string) auditlog.Log {
...
func (r *MockRedisClientImpl) FetchLatestRemediationLog(key string) auditlog.Log {
	var latestLog auditlog.Log
	latestLog = r.FetchLatestLog(key)
	if latestLog == nil {
		return nil
...
func (r *MockRedisClientImpl) FetchHeadInClientRequestList(key string) clientrequest.ClientRequestPacket {
	existsReply, _ := redis.Bool(r.client.Do(string(exists), key))
	if !existsReply {
...
func (r *MockRedisClientImpl) FetchTransactionless(key string, objectType ObjectType) interface{} {
...
func (r *MockRedisClientImpl) DeleteByPrefix(prefix string) {
...
func (r *MockRedisClientImpl) fetch(key string, objectType ObjectType, increment int) interface{} {
